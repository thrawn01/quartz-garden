...
```
This document describes a team building exercise for technical presentations and their presenters. The goal of the exercise is to prepare them for hostile audiences, particularly senior-level management and stakeholders. 
```
...
```
The presenter is expected to stay on message and maintain a steady, forward momentum during the presentation. They should concisely and persistently reassert the primary conclusion of their presentation during the Q&A session, while ignoring personal attacks and avoiding digressions. The presenter should also refuse to engage in exploratory jaunts into minutia or philosophical void.
```

The other two individuals in the exercise are intended (in summary) too preform the following.

```
* Ask several questions that probe deeply into an unknowable area of the proposed solution. 
* Take the role of a belligerent which combines emotional outbursts, foul language, and ad-hominem attacks which raise the level of stress, call into question the presenter's competence and undermine their credibility.
* Questions that challenge the presenter's assumptions or assert that the true number is 2-5x higher based on the inquisitor's understanding of an unrelated system.
* Two to three philosophical questions that are so large in scope they cannot be solved within the time allowed.
* The Belligerent may prepare, by choosing to argue from the standpoint of some cherished doctrine.
```

The document then goes on to provide a list of approved condemnatory arrangements which are directly quoted from previous company presentations and technical events. Some of these are:

```
- Only a fucking idiot would think X
- I can't believe we still haven't solved this bullshit in the year X
- X is a fucking joke
```

I find this appalling, and dis hearting to say the least. But I what really breaks it for me is the intent of this document comes from a good place. As it's author explained to me.
